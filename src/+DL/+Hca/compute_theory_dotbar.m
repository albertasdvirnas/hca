function [theory, header] = compute_theory_dotbar(name, recSite, pxSize, psfSigmaWidth_bps, k, m)
...
%   fprintf('Generating theory %d bp out of %d\n', j, n-k+1)
  ...
  [~, refDotsBpRes] = restrict(ts(j:j+k-1)', recSite(:)', round(length(recSite)/2));
  x = zeros(length(j:j+k-1), 1);
  x(refDotsBpRes(2:end)) = pxSize;
...
if k >= m % if k < m, there are not enough points on theory to compute more px values
  ...
  [~, refDotsBpRes] = restrict(ts(j+1:n)', recSite(:)', round(length(recSite)/2));
  x = zeros(length(j+1:n), 1);
  x(refDotsBpRes(2:end)) = pxSize;
