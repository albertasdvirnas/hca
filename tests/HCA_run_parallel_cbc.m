function [hcaStruct] = HCA_run_parallel(setsFile, theories, kymoFile, numFrames,hcaStruct)
    % HCA_Gui
    ...
    if nargin < 5
       ...
       lengths =  cellfun(@(x) length(x.rawBarcode),barcodeGen);
      intensity =  cellfun(@(x) mean(x.rawBarcode),barcodeGen);
        figure,plot(lengths,intensity,'x')
        xlabel('lengths')
        ylabel('intensity')
        
        % limit the range of lengths for consensus generation
          % generate consensus
...
%         [consensusStruct,sets] = select_consensus(consensusStructs,sets);
       ...
       import CBT.Hca.Export.export_cbc_compatible_consensus;
        export_cbc_compatible_consensus(consensusStruct,consensusStructs,barcodeGen,kymoStructs,sets);
