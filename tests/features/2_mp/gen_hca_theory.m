function [ theory, header] = compute_theory_barcode( name,sets)
    ...
    try 
        sets.start:sets.stop;
    catch
         sets.start = 1;
         sets.stop = length(chr1.Data);
    end
    ...
    data =  chr1.Data(sets.start:sets.stop);
    ...
    ts = [ones(extraL,1);data; ones(extraR,1)];
    if circular % so if circular, we add the theory data instead of simple ones
        ts(1:extraL) = data(end-extraL+1:end);
        ts(end-extraR+1:end) =  data(1:extraR);
    end
	...
	n = length(ts);
    nSeq = length(data); % theory + extra things on the left and right
